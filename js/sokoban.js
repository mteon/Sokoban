function Sokoban() {
    ...
    this.initTable = function (obj, lvl) {
        ...
        for (let ob of obj.cells) {
            ...
            for (let j = 0; j < ob.length; j++) {
                $('<td />').appendTo(tr).css({'height': '30px', 'width': '30px', 'color': 'rgba(0, 0, 0, 0)'});
                this.tableGrid[i][j] = ob[j] !== " " || undefined ? ob[j] : '+';

                switch (ob[j]) {
                    case '#': // mur
                        this.color({x: j, y: i}, 'black');
                        break;
                    case '@': // joueur
                        this.playerPos.y = i;
                        this.playerPos.x = j;
                        this.color({x: j, y: i}, 'red');
                        break;
                    case '.': // point d'arrivée
                        this.goals.push({x: j, y: i});
                        this.color({x: j, y: i}, 'gold');
                        break;
                    case '*': // bloc sur un point d'arrivée
                        this.goals.push({x: j, y: i});
                        this.color({x: j, y: i}, 'orange');
                        break;
                    case '$': // bloc
                        this.color({x: j, y: i}, 'darkorange');
                        break;
                }
            }
            i++;
    ...
    this.checkMove = function (move) {
        ...
        if (futurePos !== '#' && !carryingBlockIntoWall) {
            ...
            this.tableGrid[this.playerPos.y][this.playerPos.x] = '@';
            this.color({x: this.playerPos.x, y: this.playerPos.y}, 'blue');
        }
        ...
        for (let i = 0; i < this.tableGrid.length; ++i) {
            for (let j = 0; j < this.tableGrid[i].length; ++j) {
                if (this.tableGrid[i][j] === '$') this.color({y: i, x: j}, 'darkorange');
                if (this.tableGrid[i][j] === '.') this.color({y: i, x: j}, 'gold');
                if (this.tableGrid[i][j] === '*') this.color({y: i, x: j}, 'orange');
            }
        }
    }
}
...
let json = {
    ...
    "levels": [{
        ...
        "total_balls": 1,
        "width": 7
    }, {"cells": ["######", "#@ $ .#", "######"], "height": 3, "id": "63", "total_balls": 1, "width": 7}],
    "max_height": 20,
    "max_width": 29,
    "title": "Spiros 05",
    "total": 60
...
function loadCurrentLevel(data) {
    for (let i = 0; i < json.levels.length; i++) {
        if (json.levels[i] === data.level) {
            levelSelected = true;
            sokoban = new Sokoban();
            sokoban.initTable(json.levels[i], json.levels[i].id);
        }
